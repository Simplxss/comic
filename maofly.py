...
def downloadImage(page_filename, page_url):
    res = requests.get(page_url, headers={
                       'Referer': 'https://www.maofly.com/'}, verify=False)
...
def auto(id, cname):
    ...
    response = requests.get(
        f'https://www.maofly.com/manga/{id}.html', verify=False)   # verify去除ssl认证
    ...
    for chapter_name, chapter_url in zip(chapter_name_list, chapter_url_list):
        ...
        response = requests.get(chapter_url, verify=False)   # verify去除ssl认证
        rule = r'img_data = "(.*?)"'  # 正则规则
        img_data = re.findall(rule, response.content.decode())[0]

        result = js.call("LZString.decompressFromBase64", img_data)

        page_url_list = result.split(',')
        page = 1
