...
def auto(id, cname):
    ...
    for chapter_name, chapter_url in zip(chapter_name_list, chapter_url_list):
        ...
        content = requests.get(f'{DOMAIN}{chapter_url}', headers=headers, verify=False)
        ...
        for i in range(1, int(image_count)):
            content = requests.get(
                f'{DOMAIN}/{id}/chapterfun.ashx?cid={cid}&page={i}&key=&language=1&gtk=6&_cid={cid}&_mid={mid}&_dt={date}&_sign={sign}', headers=headers, verify=False)
