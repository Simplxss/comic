...
aK = deque([i[1:-1]
           for i in re.findall(r"var aK=\[(.*?)\];", js.text)[0].split(",")])
table = re.findall(
    r"G==ag\((0x[0-9a-f]*?)\)&&\(I=ag\((0x[0-9a-f]*?)\)\),", js.text)


def cal(N): return int(re.search(r'\d+', decode_str(N).decode()).group())


while True:
    try:
        if -cal(0x928) / 0x1 + cal(0x62b) / 0x2 + -cal(0x699) / 0x3 * (cal(0x52a) / 0x4) + cal(0x5b0) / 0x5 + -cal(0xd85) / 0x6 * (cal(0x6e2) / 0x7) + -cal(0xab6) / 0x8 + cal(0xbdf) / 0x9 == 0xd2942:
            break
        else:
            aK.rotate(-1)
    except:
...
KEY_TABLE = {decode_str(int(k, 0)).decode(): decode_str(int(v, 0))
             for k, v in table}
...
def auto(id, cname):
    ...
    for chapter_name, chapter_url in zip(chapter_name_list, chapter_url_list):
        ...
        startimg, enc_code1, enc_code2, keyType, imgKey = pattern.findall(mh_info)[
            0]
        ...
        try:
            ...
            _tka_ = decrypt_ecb(
                enc_code2,
                b"0aW5swj3TBKSrfBU",
            ).decode("gbk")  # _tka_ + mh_info["pageid"]
        ...
        for page in range(int(startimg), int(_tkb_) + 1):
            if keyType != '0' and keyType != "":
                task_list.append(
                    pool.submit(
                        downloadImage, f".\\{page}.webp", f"{DOMAIN_IMG}/comic/{_tka_}{page:0>4d}.enc.webp", chapter_url, KEY_TABLE[keyType])
                )
            elif imgKey != "":
                task_list.append(
                    pool.submit(
                        downloadImage, f".\\{page}.webp", f"{DOMAIN_IMG}/comic/{_tka_}{page:0>4d}.enc.webp", chapter_url, imgKey)
                )
            else:
                task_list.append(
                    pool.submit(
                        downloadImage, f".\\{page}.jpg", f"{DOMAIN_IMG}/comic/{_tka_}{page:0>4d}.jpg", chapter_url)
                )
            time.sleep(0.1)
